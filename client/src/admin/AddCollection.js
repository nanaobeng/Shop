...
const AddCollection = () => {
    ...
    const clickSubmit = (e) => {
        ...
        setError('')
        setSuccess(false)
        createCollection(user._id,token,{name})
        .then(data => {
            if(data.error){
                setError(data.error)
            }
    ...
    const newCollectionForm = ()=> (
        <form onSubmit={clickSubmit}>
            ...
            <button className="btn btn-outline-primary">Create Collection</button>
        </form>
    )
    ...
    const showError = () => {
        if(error) {
            return <h3 className="text-danger alert alert-danger"> Collection should be unique</h3>
        }
    }
